...
void output(const char prompt[], const char d[])
{
	...
	for (int i = 1; i <= last_digit_pos; i++) // 遍历打印后面的每一位小数
	{
		cout << (int)d[i];
	}
	cout << endl;
...
void calc_next(char d_cur[], char d_next[])
{
	...
	{
		int divided = 10 * remainder + d_cur[i];
		d_next[i] = divided / 2;
		remainder = divided % 2;
	}
}
...
{
	for (int i = 1; pure_decimal[i] != '\0' && i <= N_POW; i++)
	{
		if (pure_decimal[i] == '1')
		{
			int forward = 0;
			...
			{
				int sum = power_table[i][j] + out[j] + forward;
				out[j] = sum % 10;
				forward = sum / 10;
			}
		}
	...
	while(true)
	{
		...
		else
		{
			...
			if (!valid)
				...
				if (strlen(pure_decimal) > N_POW + 1)
					pure_decimal[N_POW + 1] = '\0';
				break;
